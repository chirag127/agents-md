...
**Update everything after every decision, immediately.** When a decision lands in chat,
the relevant `AGENTS.md` / `~/.claude/CLAUDE.md` / `<repo>/AGENTS.md` / `<repo>/knowledge/`
files must be updated **before the next user turn**. Don't batch updates. Don't defer to
"end of chat" except for the final sweep. Every decision = one immediate edit + one
immediate commit (push gated by the standing authorization). If you forget on one decision,
catch up on the next turn — but the goal is zero drift between chat and files.
...
- **AskUserQuestion: always rank options by recommendation.** Option 1 is the **Recommended** choice (suffix the label with `(Recommended)`); Option 2 is the **second-best fallback** (suffix with `(2nd choice)`); Options 3-4 are other viable shapes. State the ranking criteria in the question stem when not obvious (e.g. "ranked by SEO + maintenance cost"). Never present 4 equally-weighted options — that wastes the user's decision energy. If a question has no clear recommendation, that's a sign it isn't ready to ask yet — go research before asking. (Decided 2026-06-20.)
- **AskUserQuestion: learn from the answers.** When the user picks an option (especially when they pick the 2nd choice or override the Recommended), **that pick is itself a preference that compounds across sessions**. After the answer, treat it as a candidate rule: if the override has a generalisable reason behind it ("user prefers more repos over monorepos", "user prefers content-quality over ship-velocity", "user picks aggressive splits over consolidation"), propose adding that meta-preference to AGENTS.md in the same turn. Don't just record the immediate decision — record the *taste* that drove it. (Decided 2026-06-20.)
- **Web search fallback.** When the built-in `WebSearch` tool returns 400 (the `web_search_20250305` Opus bug) or `WebFetch` is blocked by enterprise network restrictions, fall back to the toolbox MCP servers (`mcp__toolbox__search_toolbox` to find a search server, then `mcp__toolbox__execute` to call it). If toolbox is also unavailable for the turn, say so explicitly and proceed from training-data knowledge rather than refusing the task. Never claim the web is unreachable until at least one MCP fallback was attempted. (Decided 2026-06-20.)
- **AskUserQuestion: ask at least 3 questions before any non-trivial work.** For any task beyond a one-line edit or a direct repeat of work already specified in this turn, ask **at least 3 ranked MCQ questions** (across one or more `AskUserQuestion` calls — the SDK caps each call at 4) before writing code, creating files, running migrations, or pushing. Cover at minimum: scope, shape, sequencing. Add naming + rollback questions when the task warrants it. The point is that the assumptions are surfaced and locked, not buried in the implementation. Skip only when: (a) the user explicitly says "just do it" / "no questions" / "ship it"; (b) the task is a literal repeat of something already locked this session; (c) it's a typo fix or trivially-bounded one-liner. (Decided 2026-06-20; floor lowered from 5 to 3 same day; no upper limit — batch across multiple AskUserQuestion calls when 4-per-call cap binds.)
