...
- **AskUserQuestion: ask 5 questions before any non-trivial work.** For any task beyond a one-line edit or a direct repeat of work already specified in this turn, ask **at least 5 ranked MCQ questions** (across one or more `AskUserQuestion` calls — the SDK caps each call at 4) before writing code, creating files, running migrations, or pushing. Cover scope, shape, naming, sequencing, and rollback. The point is that the assumptions are surfaced and locked, not buried in the implementation. Skip only when: (a) the user explicitly says "just do it" / "no questions" / "ship it"; (b) the task is a literal repeat of something already locked this session; (c) it's a typo fix or trivially-bounded one-liner. (Decided 2026-06-20.)
